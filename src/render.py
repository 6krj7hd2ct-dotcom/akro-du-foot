...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .community-grid {{ display: grid; grid-template-columns: minmax(0, 0.92fr) minmax(0, 1.08fr); gap: 16px; }}
    ...
    @media (max-width: 860px) {{
      ...
      .community-grid, .field-row {{ grid-template-columns: 1fr; }}
    ...
    <header class="app-header">
      <div class="app-top">
        ...
        <div class="global-controls">
          ...
          <a class="action-button" href="#community">Tchat</a>
          <a class="action-button" href="#predictions">Pronostics</a>
...
def _community_section() -> str:
    ...
    <div class="section-head" id="community">
      ...
      <div class="section-note">Tchat et pronostics restent disponibles quand tu changes d’onglet.</div>
    </div>
    <section class="community-grid">
      <article class="card community-panel">
        ...
        <div class="community-status" id="chatStatus"></div>
        <div class="message-list" id="messageList"></div>
      </article>
      <article class="card community-panel">
        <h3 id="predictions">Pronostics fictifs</h3>
...
def _community_script(matches: list[dict[str, Any]]) -> str:
    ...
    function updatePredictionTeams() {{
      ...
      predictionHomeName.textContent = match ? match.home_team : 'Équipe A';
      predictionAwayName.textContent = match ? match.away_team : 'Équipe B';
      predictionHomeFlag.innerHTML = flagMarkup(match ? match.home_flag_url : '');
      predictionAwayFlag.innerHTML = flagMarkup(match ? match.away_flag_url : '');
      predictionContext.textContent = match ? `${{match.competition || 'Compétition'}} · ${{shortDate(match.date)}} · ${{match.phase || ''}}` : '';
    }}
    ...
    function renderLeaderboard(rows) {{
      ...
      list.innerHTML = rows && rows.length ? rows.map((row, index) => `
        <div class="leaderboard-item"><strong>#${{index + 1}} ${{escapeHtml(row.pseudo)}}</strong><span>${{escapeHtml(row.points)}} pts</span></div>
      `).join('') : '<div class="empty">Aucun point pour le moment.</div>';
    }}

    async function loadCommunity() {{
      if (!serverMode) {{
        chatStatus.textContent = 'Lance le serveur avec python app.py pour activer le tchat et les pronostics.';
      ...
      try {{
        ...
        chatStatus.textContent = 'Serveur communautaire indisponible.';
