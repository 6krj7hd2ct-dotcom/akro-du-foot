...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .app-top {{ position: relative; z-index: 101; display: grid; grid-template-columns: minmax(0, 1fr); gap: 20px; align-items: start; }}
    ...
    .global-controls {{ position: relative; z-index: 102; display: flex; flex-direction: row; flex-wrap: nowrap; gap: 16px; align-items: center; justify-content: space-between; width: 100%; max-width: min(1040px, 100%); }}
    .global-actions {{ display: flex; flex: 0 0 auto; flex-wrap: nowrap; gap: 10px; justify-content: flex-end; align-items: center; padding-left: 0; white-space: nowrap; }}
    .global-search {{ position: relative; z-index: 9998; flex: 1 1 auto; min-width: 0; max-width: none; }}
    ...
    .search-results {{ position: absolute; z-index: 9999; top: calc(100% + 10px); left: 0; right: 0; width: 100%; display: none; max-height: min(430px, 70vh); overflow-y: auto; padding: 8px; border: 1px solid rgba(255,255,255,0.14); border-radius: 16px; background: rgba(5,16,31,0.98); box-shadow: var(--shadow); backdrop-filter: blur(16px); }}
    ...
    .favorite-matches-block {{ display: grid; gap: 10px; margin-top: 4px; padding-top: 12px; border-top: 1px solid rgba(255,255,255,0.10); }}
    .favorite-matches-block h4 {{ margin: 0; font-size: 16px; line-height: 1.2; }}
    .favorite-match-list {{ display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; }}
    .favorite-match-list .empty {{ grid-column: 1 / -1; }}
    ...
    @media (max-width: 860px) {{
      ...
      .global-controls {{ width: 100%; max-width: none; flex-direction: column; align-items: stretch; gap: 10px; }}
      ...
      .community-grid, .community-predictions, .field-row, .follow-list, .favorite-match-list, .profile-summary {{ grid-template-columns: 1fr; }}
...
def _news_script(worldcup_data: dict[str, Any], champions_data: dict[str, Any] | None, leagues_data: dict[str, Any] | None = None) -> str:
    ...
    async function refreshNews(kind) {{
      ...
      try {{
        const params = new URLSearchParams({{competition: kind, focus: kind === 'worldcup' ? (document.getElementById('worldcupFocusSelect')?.value || '') : '', league: kind === 'leagues' ? selectedLeagueKey() : ''}});
...
def _community_script(matches: list[dict[str, Any]]) -> str:
    ...
    function normalizeTeamName(value) {{
      return String(value || '').toLocaleLowerCase('fr-FR').normalize('NFD').replace(/[\\u0300-\\u036f]/g, '').replace(/[-_.]/g, ' ').replace(/\\s+/g, ' ').trim();
    }}
    ...
    function uniqueTeams(values) {{
      ...
      return (values || []).map(value => String(value || '').trim()).filter(value => {{
        ...
        if (!key || key === 'a determiner' || seen.has(key)) return false;
        seen.add(key);
        return true;
      }});
    }}

    function profileFocusTeams() {{
      if (!pseudoValue()) return [];
      const localFocus = [
        localStorage.getItem('akrodufoot:focus:worldcup'),
        localStorage.getItem('akrodufoot:focus:champions')
      ];
      Object.keys((typeof LEAGUES_DATA !== 'undefined' && LEAGUES_DATA.leagues) || {{}}).forEach(key => {{
        localFocus.push(localStorage.getItem(`akrodufoot:league-focus:${{key}}`));
      }});
      return uniqueTeams(localFocus);
    }}

    function isFavoriteMatch(match, teams) {{
      const home = normalizeTeamName(match.home_team);
      const away = normalizeTeamName(match.away_team);
      return teams.some(team => {{
        const normalized = normalizeTeamName(team);
        return normalized && (home === normalized || away === normalized);
      }});
    }}
    ...
    function renderFavoriteMatches(matches) {{
      ...
      const upcoming = (matches || [])
        .filter(match => !match.completed && matchTimestamp(match) >= now)
        .filter(match => isFavoriteMatch(match, teams))
        .sort((a, b) => matchTimestamp(a) - matchTimestamp(b))
        .slice(0, 3);
      favoriteMatchesList.innerHTML = upcoming.length
        ? upcoming.map(followMatchCard).join('')
        : '<div class="empty">Aucun prochain match favori disponible.</div>';
    }}

    function liveUpdatedText(date = new Date()) {{
      return `Dernière mise à jour : ${{date.toLocaleTimeString('fr-FR', {{hour:'2-digit', minute:'2-digit', second:'2-digit'}})}}`;
    }}

    function setLiveRefreshState(loading, message = '', isError = false) {{
      liveRefreshButton?.classList.toggle('is-loading', loading);
      if (liveRefreshButton) liveRefreshButton.disabled = loading;
      if (liveUpdateMeta) {{
        liveUpdateMeta.textContent = message || liveUpdatedText();
        liveUpdateMeta.classList.toggle('is-error', Boolean(isError));
      }}
    }}
    ...
    async function loadCommunity() {{
      if (!serverMode) {{
        ...
        setLiveRefreshState(false, 'Mode fichier statique : scores en cache');
        return;
      }}
      try {{
        const response = await fetch(`/api/community?ts=${{Date.now()}}`, {{cache: 'no-store'}});
        if (!response.ok) throw new Error('community refresh failed');
        const data = await response.json();
        ...
        updatePredictionButton();
        setLiveRefreshState(false, liveUpdatedText());
      }} catch (error) {{
        console.error('[community] refresh error', error);
        predictionStatus.textContent = 'Serveur communautaire indisponible.';
        setLiveRefreshState(false, 'Actualisation impossible pour le moment', true);
      }}
    }}

    async function refreshCommunityLive(force = false) {{
      const now = Date.now();
      if (liveRefreshInFlight) return;
      const minDelay = force ? 5000 : LIVE_MIN_MANUAL_REFRESH_MS;
      if (now - lastLiveRefreshAt < minDelay) return;
      liveRefreshInFlight = true;
      lastLiveRefreshAt = now;
      setLiveRefreshState(true, 'Actualisation des scores...');
      try {{
        await loadCommunity();
      }} finally {{
        liveRefreshInFlight = false;
        liveRefreshButton?.classList.remove('is-loading');
        if (liveRefreshButton) liveRefreshButton.disabled = false;
      }}
    }}
    ...
    liveRefreshButton?.addEventListener('click', () => refreshCommunityLive(true));
    document.addEventListener('akro:focus-change', () => renderFavoriteMatches(communityMatches));
    document.addEventListener('akro:league-focus-change', () => renderFavoriteMatches(communityMatches));
    ...
    renderMatchOptions(DASHBOARD_MATCHES);
    loadCommunity();
    setInterval(() => refreshCommunityLive(false), LIVE_AUTO_REFRESH_MS);
  </script>"""
