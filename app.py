...
def _hall_of_fame_supabase_profiles_payload() -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        profiles.append({
            ...
            "national_team": str(row.get("national_team") or "").strip(),
            "ballon_dor_count": _coach_stat_int(row.get("ballon_dor_count")),
            "ballon_dor_years": _coach_json_list(row.get("ballon_dor_years")),
            "world_cup_titles": _coach_json_list(row.get("world_cup_titles")),
            "euro_titles": _coach_json_list(row.get("euro_titles")),
            "copa_america_titles": _coach_json_list(row.get("copa_america_titles")),
            "champions_league_titles": _coach_json_list(row.get("champions_league_titles")),
            "fifa_awards": _coach_json_list(row.get("fifa_awards")),
            "major_records": _coach_json_list(row.get("major_records")),
            "iconic_moments": _coach_json_list(row.get("iconic_moments")),
            "nickname": str(row.get("nickname") or "").strip(),
            "legacy": str(row.get("legacy") or "").strip(),
            "playing_style": str(row.get("playing_style") or "").strip(),
            "why_legend": str(row.get("why_legend") or "").strip(),
